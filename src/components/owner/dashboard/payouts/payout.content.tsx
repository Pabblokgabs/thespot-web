import {
	Button,
	Card,
	Tag,
	Dropdown,
	Statistic,
	List,
	Space,
	Typography,
	Row,
	Col,
} from "antd";
import {
	PlusOutlined,
	DownOutlined,
	DollarOutlined,
	EllipsisOutlined,
} from "@ant-design/icons";
import { TransactionHistory, PayoutsSettings } from "./transaction.history";
import {
	FaCalendarCheck,
	FaChartLine,
	FaClock,
	FaCreditCard,
	FaPaypal,
	FaTrash,
	FaUniversity,
} from "react-icons/fa";
const { Title } = Typography;

const PayoutsContent = () => {
	return (
		<div className="md:p-6">
			<div className="flex px-4 md:px-0 justify-between items-center mb-6 mt-4">
				<Title level={4} className="m-0 hidden md:block">
					Payouts & Transactions
				</Title>
				<h4 className="m-0 text-xl block md:hidden">Transactions</h4>
				<Space>
					<Dropdown
						menu={{
							items: [
								{ key: "1_day", label: "Today" },
								{ key: "7_days", label: "Last 7 Days" },
								{ key: "30_days", label: "Last 30 Days" },
								{ key: "1_year", label: "This Year" },
							],
						}}
						trigger={["click"]}
					>
						<Button className="cursor-pointer whitespace-nowrap !rounded-button">
							Last 30 Days <DownOutlined />
						</Button>
					</Dropdown>
					{/* <Button
						type="primary"
						icon={<i className="fas fa-download"></i>}
						className="cursor-pointer whitespace-nowrap !rounded-button"
					>
				/......		Export Statement
					</Button> */}
				</Space>
			</div>
			<Row gutter={[24, 24]}>
				{/* Stats Cards */}
						<Col xs={24} sm={12} md={6}>
							<Card className="h-full shadow-sm pl-4">
								<Statistic
									title="Available Balance"
									value={24580}
									prefix={<DollarOutlined className="text-blue-600 mr-1" />}
									valueStyle={{ color: "#2563EB" }}
									precision={2}
								/>
								<div className="mt-2">
									<Button
										type="primary"
										block
										className="cursor-pointer whitespace-nowrap !rounded-button"
									>
										Withdraw Funds
									</Button>
								</div>
							</Card>
						</Col>
						<Col xs={24} sm={12} md={6}>
							<Card className="h-full shadow-sm">
								<Statistic
									title="Pending Balance"
									value={3250}
									prefix={
										<FaClock className="fas fa-clock text-blue-600 mr-1" />
									}
									valueStyle={{ color: "#2563EB" }}
									precision={2}
								/>
								<div className="mt-2 text-xs text-gray-500">
									Available in 3-5 business days
								</div>
							</Card>
						</Col>
						<Col xs={24} sm={12} md={6}>
							<Card className="h-full shadow-sm">
								<Statistic
									title="Total Earnings"
									value={158920}
									prefix={<FaChartLine className="text-blue-600 mr-1" />}
									valueStyle={{ color: "#2563EB" }}
									precision={2}
								/>
								<div className="mt-2 text-xs text-gray-500">
									<span className="text-green-500">↑ 12%</span> from last month
								</div>
							</Card>
						</Col>
						<Col xs={24} sm={12} md={6}>
							<Card className="h-full shadow-sm">
								<Statistic
									title="Next Payout"
									value={8750}
									prefix={<FaCalendarCheck className="text-blue-600 mr-1" />}
									valueStyle={{ color: "#2563EB" }}
									precision={2}
								/>
								<div className="mt-2 text-xs text-gray-500">
									Scheduled for June 15, 2025
								</div>
							</Card>
						</Col>

				{/* Transaction History */}
				<Col xs={24}>
					<TransactionHistory />
				</Col>
				{/* Payment Methods */}
				<Col xs={24} md={12}>
					<Card
						title={
							<div className="flex items-center">
								<FaCreditCard className="fas fa-credit-card mr-2 text-blue-600" />{" "}
								Payment Methods
							</div>
						}
						extra={
							<Button
								type="primary"
								icon={<PlusOutlined />}
								className="cursor-pointer whitespace-nowrap !rounded-button"
							>
								Add Method
							</Button>
						}
						className="border-none m-0 md:border shadow-none md:shadow-md"
					>
						<List
							className="hidden md:block"
							itemLayout="horizontal"
							dataSource={[
								{
									id: 1,
									type: "Bank Account",
									name: "Chase Business Account",
									number: "****1234",
									isDefault: true,
								},
								{
									id: 2,
									type: "Bank Account",
									name: "Wells Fargo Account",
									number: "****5678",
									isDefault: false,
								},
								{
									id: 3,
									type: "PayPal",
									name: "Business PayPal",
									number: "john.doe@email.com",
									isDefault: false,
								},
							]}
							renderItem={(item) => (
								<List.Item
									actions={[
										<Button
											type="text"
											key="edit"
											className="cursor-pointer whitespace-nowrap !rounded-button"
										>
											Edit
										</Button>,
										<Button
											type="text"
											key="remove"
											className="cursor-pointer whitespace-nowrap !rounded-button"
										>
											Remove
										</Button>,
									]}
								>
									<List.Item.Meta
										avatar={
											<i
												className={`fas fa-${
													item.type === "PayPal" ? "paypal" : "university"
												} text-2xl text-blue-600`}
											></i>
										}
										title={
											<div className="flex items-center">
												{item.name}{" "}
												{item.isDefault && (
													<Tag
														color="blue"
														className="ml-2 whitespace-nowrap !rounded-button"
													>
														Default
													</Tag>
												)}
											</div>
										}
										description={
											<div className="text-gray-500">
												{item.type} ending in {item.number}
											</div>
										}
									/>
								</List.Item>
							)}
						/>

						<div className="md:hidden">
							{[
								{
									id: 1,
									type: "Bank Account",
									name: "Chase Business Account",
									number: "****1234",
									isDefault: true,
								},
								{
									id: 2,
									type: "Bank Account",
									name: "Wells Fargo Account",
									number: "****5678",
									isDefault: false,
								},
								{
									id: 3,
									type: "PayPal",
									name: "Business PayPal",
									number: "john.doe@email.com",
									isDefault: false,
								},
							].map((item) => (
								<div key={item.id} className="flex">
									<div className="flex-1">
										<div className="flex items-center gap-2">
											{item.type === "PayPal" ? <FaPaypal /> : <FaUniversity />}
											<h4>{item.name}</h4>
											{item.isDefault && <Tag color="blue">Default</Tag>}
										</div>
										<div className="text-gray-500">
											{item.type} ending in {item.number}
										</div>
									</div>
									<Button type="text">
										<FaTrash className="text-red-400" />
									</Button>
								</div>
							))}
						</div>
					</Card>
				</Col>
				{/* Payout Schedule */}
				<Col xs={24} md={12}>
					<PayoutsSettings />
				</Col>
			</Row>
		</div>
	);
};

export default PayoutsContent;
